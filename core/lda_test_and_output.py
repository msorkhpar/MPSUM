...
'''
# I should comment this and run evaluate.sh to get results for 10 times.
# The results will be in ESBM-eval folder 
# the fscore and NDGC results is in the root.  
...
def entity_summarization(kb, kb_name, tfidf_flag):
    ...
    if kb == 'dbpedia':
        num, base_num, skip = 25, 140, 100
    elif kb == 'lmdb':
        num, base_num, skip = 10, 165, 40
    else:
        return

    make_es_lda_output_subdir(kb, num, base_num, tfidf_flag)
    for i in range(num):
        matched_total_lines = rank_rdf_triples(i + skip, object_corpus_list, kb_path, corpus_word)
        construct_es_lda_output(kb, i + 1, base_num, matched_total_lines, tfidf_flag)
