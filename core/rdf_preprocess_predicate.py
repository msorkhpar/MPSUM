...
def form_and_store_predicate_set(predicate_doc_list_db, predicate_set, kb_name):
    ...
    with open(os.path.join(rdf_preprocess_dict.coredir, 'predicate_corpus_list_' + kb_name + '.json'), 'w+',
              encoding='utf-8') as f:
...
def predicate_extract(triple):
    ...
    with open(os.path.join(rdf_preprocess_dict.coredir, 'predicate_extract_temp.nt'), 'w',
              encoding='utf-8') as extract_file:
...
def retrieve_object_corpus_list(kb_name):
    with open(os.path.join(rdf_preprocess_dict.coredir, 'predicate_corpus_list_' + kb_name + '.json'), 'r',
              encoding='utf-8') as f:
