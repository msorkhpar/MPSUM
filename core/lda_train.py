...
def apply_lda_model(kb_name, num, n_top_words):
    ...
    with open(os.path.join(rdf_preprocess_dict.coredir, 'object_output_' + kb_name + '.txt'),
...
def form_and_store_corpus_word(kb_name, num, n_top_words):
    ...
    with open(os.path.join(rdf_preprocess_dict.coredir, 'corpus_word_' + kb_name + '.json'), 'w+',
              encoding='utf-8') as f:
        json.dump(corpus_word, f)


def retrieve_corpus_word(kb_name):
    with open(os.path.join(rdf_preprocess_dict.coredir, 'corpus_word_' + kb_name + '.json'), 'r',
              encoding='utf-8') as f:
